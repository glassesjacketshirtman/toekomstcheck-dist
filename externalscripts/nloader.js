...
!(function(a) {
  ...
  function l(a) {
    ...
    function C() {
      Q[V] &&
        Q[V].stopPageInfo &&
        (Q[V].stopPageInfo(), delete Q[V].stopPageInfo);
    }
    ...
    function J() {
      switch ((Q[V] && Q[V].firstRun && R(), U.type)) {
        case "close":
          Q[V].closeRequestCallback
            ? m(V, "closeRequestCallback", Q[V].iframe)
            : n(U.iframe);
          break;
        case "message":
          x(w(6));
  ...
  function u(a, b, c, d, e) {
    function f() {
      ...
      h(
        ...
        c.contentWindow.postMessage(L + b, e);
    }
  ...
  function w(c, d) {
    ...
    function i() {
      switch (
        (h(
          x,
          "IFrame scrolling " +
            (Q[x] && Q[x].scrolling ? "enabled" : "disabled") +
            " for " +
            x
        ),
        (c.style.overflow =
          !1 === (Q[x] && Q[x].scrolling) ? "hidden" : "auto"),
        Q[x] && Q[x].scrolling)
      ) {
        case !0:
          c.scrolling = "yes";
    ...
    function k() {
      ("number" != typeof (Q[x] && Q[x].bodyMargin) &&
        "0" !== (Q[x] && Q[x].bodyMargin)) ||
        ((Q[x].bodyMarginV1 = Q[x].bodyMargin),
        (Q[x].bodyMargin = Q[x].bodyMargin + "px"));
    }
      ...
      Function.prototype.bind &&
        ...
        (Q[x].iframe.iFrameResizer = {
          ...
          sendMessage: function sendMessage(a) {
            (a = JSON.stringify(a)),
              u("Send Message", "message:" + a, Q[x].iframe, x);
          },
        });
    }
    ...
    function t(a) {
      (a = a || {}),
        ...
        Q[x] &&
          (Q[x].targetOrigin =
            !0 === Q[x].checkOrigin ? s(Q[x].remoteHost) : "*");
    }
    ...
    function a() {
      function a(a) {
        ...
        Q[a] &&
          c(Q[a].iframe) &&
          (b("height") || b("width")) &&
          u("Visibility change", "resize", Q[a].iframe, a);
      }
      for (var b in Q) {
        a(b);
      }
  ...
  function E() {
    ...
    return (
      ...
      function(d, f) {
        switch (
          ((e = []), c(d), typeof f === "undefined" ? "undefined" : _typeof(f))
        ) {
          case "undefined":
          case "string":
...
function initIframe(iframe_id) {
  ...
  if (og_check == 0) {
    ...
    // og_iFrame.setAttribute("style", "border:none;min-height:" + height + "px; box-shadow: 0px 11px 24px rgba(255,0,0,.28);");
    og_iFrame.setAttribute(
      "style",
      "border:none;min-height:" + height + "px; box-shadow: none;"
    );
...
function getParameterByName(name, url) {
  ...
  return decodeURIComponent(
    results[2].replace(/\+/g, " ").replace(/%(?![0-9][0-9a-fA-F]+)/g, "%25")
  );
}
